def register_api_url_stubs  
  unless(TEST_LIVE_API)
    ...
    File.open(File.join(File.dirname(__FILE__), "..", "fixtures", "movie_videos.txt")) do |file|
      stub_request(:get, Regexp.new(Tmdb.base_api_url + '/movie/\d+/videos')).to_return(file)
    end
